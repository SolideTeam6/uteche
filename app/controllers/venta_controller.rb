...
before_action :authenticate_user!
    ...
    def create
        ...
        respond_to do |format|
            if @ventum.save
                @monedero = Monedero.find_by(student: @ventum.student)
                @monedero.cantidad = @monedero.cantidad - (@ventum.product.precio * @ventum.cantidad)
                @monedero.save

                @stock = Stock.find_by(product: @ventum.product)
                @stock.cantidad = @stock.cantidad - @ventum.cantidad
                @stock.save
    ...
    def ventum_params
        params.require(:ventum).permit(:student_id, :product_id, :cantidad)
    end
end
